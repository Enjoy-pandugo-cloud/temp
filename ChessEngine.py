...
class GameState:
    ...
    def makeMove(self, move, promotion_choice="Q"):
        """
    ...
    def getValidMoves(self):
        ...
        # Check for threefold repetition before stalemate
        if self.isThreefoldRepetition():
            self.stalemate = False  # Ensure stalemate is not set if it's a threefold repetition
            return []

        # Check for 50-move rule
        if self.isFiftyMoveRule():
            self.stalemate = False  # Ensure stalemate is not set if it's a 50-move rule
            return []

        # Check for insufficient material
        if self.isInsufficientMaterial():
            self.stalemate = False  # Ensure stalemate is not set if it's insufficient material
            return []
